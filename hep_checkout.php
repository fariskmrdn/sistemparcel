...
$hepid = $_SESSION['idhep'];
$getMPP = mysqli_query($connect,"SELECT * FROM hep_acc WHERE hep_id = '$hepid'");
$fetchMPP = mysqli_fetch_array($getMPP);

$fetchParcelCons = mysqli_query($connect,"SELECT * FROM inventory_hep WHERE consignment = '$consignment'");
...
if (isset($_POST['submit'])) {
    ...
    //first command
    $addData = "INSERT INTO `settlements_hep` (`receiver`, `tel`, `consignment`, `courier`, `time`, `handler`, `pmethod`) VALUES ('$receiver', '$tel', '$tracking', '$courier', '$time', '$handler', '$pmethod')";
    $addQuery = mysqli_query($connect,$addData);

    //sec cmd
    $deleteNorm = "DELETE FROM inventory_hep WHERE consignment = '$consignment'";
